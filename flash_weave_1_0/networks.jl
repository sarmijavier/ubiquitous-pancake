...
function run_study_analysis(study_name, folder_type)
    ...
    base_path = "./flash_weave_1_0/$study_name/$folder_type/"

    for p in percentages
        p_int = Int(p * 100)
        
        # Create output directories for networks
        out_dir_h = "$(base_path)networks/$p_int/healthy/"
        out_dir_u = "$(base_path)networks/$p_int/unhealthy/"
        mkpath(out_dir_h)
        mkpath(out_dir_u)
